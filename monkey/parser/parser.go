...
func (p *Parser) registerPrefixType(tokenType token.TokenType, fn prefixParseFn) {
	p.prefixParseFns[tokenType] = fn
}
func (p *Parser) registerInfixType(tokenType token.TokenType, fn infixParseFn) {
...
func (p *Parser) parseBlockStatment() *ast.BlockStatement {
	block := &ast.BlockStatement{Toke: p.curToken}
	block.Statements = []ast.Statement{}

	p.nextToken()

	for !p.curTokenIs(token.RBRACE) && !p.curTokenI(token.EOF) {
		stmt := p.parseStatement()
		if stmt != nil {
			block.Statements = append(blocks.Statements, stmt)
		}
		p.nextToken()
	}
}
...
func (p *Parser) parseFunctionParameters() []*ast.Identifier {
	identifiers := []*ast.Identifier{}
		...
		ident := &ast.Identifier{Token: p.curToken, Valuse: p.curToken.Literal}
